...
if __name__ == "__main__":
    ...
    plt.hist(times_avg,bins= 200, density=True)    
    plt.xlabel('Average days between missing cookies')
    plt.ylabel('Probability')
    plt.title('Rate (2 cookies per day)')
    plt.yscale('log') 
    plt.grid(True)
    plt.show()
    
    plt.hist(times,bins= 200, density=True)    
    plt.xlabel('Number of days between missing cookies')
    plt.ylabel('Probability')
    plt.title('Rate (2 cookies per day)')
    plt.yscale('log')
    plt.grid(True)
